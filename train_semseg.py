...
from SinGAN.functions import norm, colorize_mask
import numpy as np
import time
import os
...
def main():
    ...
    source_train_loader.dataset.SetEpochSize(opt.epoch_size)
    target_val_loader = CreateTrgDataLoader(opt, 'val')
    target_test_loader = CreateTrgDataLoader(opt, 'test')
    ...
    while keep_training:
        ...
        for batch_num, (source_scales, source_label) in enumerate(source_train_loader):
            ...
            if int(steps/opt.save_pics_rate) >= save_pics_int or steps == 0:
                s       = norm(source_scales[-1][0])
                sit     = norm(source_in_target[0])
        ...
        for val_batch_num, (target_images, target_labels) in enumerate(target_val_loader):
            with torch.no_grad():
                pred_softs = semseg_net(target_images.to(opt.device))
        ...
        del cm, pred_softs, pred_labels
        if epoch_num % 4 == 0:
            torch.save(semseg_net, '%s/Gst.pth' % (opt.out_))
        epoch_num += 1
    ...
    #Test:
    print('train semseg: starting test stage...')
    semseg_net.eval()
    rand_samp_inds = np.random.randint(0, len(target_test_loader.dataset), 5)
    rand_batchs = np.floor(rand_samp_inds/opt.batch_size).astype(np.int)
    cm = torch.zeros((NUM_CLASSES, NUM_CLASSES)).cuda()
    for test_batch_num, (target_images, target_labels) in enumerate(target_test_loader):
        with torch.no_grad():
            pred_softs = semseg_net(target_images.to(opt.device))
            pred_labels = torch.argmax(pred_softs, dim=1)
            cm += compute_cm_batch_torch(pred_labels, target_labels, IGNORE_LABEL, NUM_CLASSES)
            if test_batch_num in rand_batchs:
                t        = norm(target_images[0])
                t_lbl    = colorize_mask(target_labels[0])
                pred_lbl = colorize_mask(pred_labels[0])
                opt.tb.add_image('Test/target', t, test_batch_num)
                opt.tb.add_image('Test/target_label', t_lbl, test_batch_num)
                opt.tb.add_image('Test/prediction_label', pred_lbl, test_batch_num)
    iou, miou = compute_iou_torch(cm)
